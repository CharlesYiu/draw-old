...
function load() {
    function loadAction(safe) {
        ...
        document.getElementById(elementIds.advancedSettingsForceComputerButton).onclick = () => {
        ...
        if (releaseNotes.notes !== [] && localStorage.getItem(`viewed-release-note-mobile-${releaseNotes.version}`) === null) {
            ...
            document.getElementById(elementIds.releaseNotesDoneButton).onclick = () => {
                releaseNotesDiv.hidden = true
                localStorage.setItem(`viewed-release-note-mobile-${releaseNotes.version}`, "yes")
            }
        }
        console.log("Loaded")
    }
    function loadResources() {
        ...
        tries += 1
        fetch(`${window.location.protocol}//${window.location.host}${mobilePath}config.json`)
        ...
        fetch(`${window.location.protocol}//${window.location.host}${mobilePath}releasenotes.json`)
        .then(response => response.json())
        .then(json => {
            releaseNotes = json
